...
interface IValidationHook {
    ...
    /// @return isValid Whether the signature is valid or not. The hook MUST still return true even if no extra validation is done, since this value overrides the validity internally.
    function afterValidateUserOp(
        bytes32 keyHash,
        PackedUserOperation calldata userOp,
        bytes32 userOpHash,
        bytes calldata hookData
    ) external view returns (bytes4 selector, bool isValid);

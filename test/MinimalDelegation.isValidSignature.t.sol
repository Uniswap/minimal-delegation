...
contract MinimalDelegationIsValidSignatureTest is DelegationHandler, HookHandler, ERC1271Handler {
     ...
     * 4. Is root key so fallback to raw ECDSA recover
     * = valid signature
     */
    /// forge-config: default.isolate = true
    /// forge-config: ci.isolate = true
    function test_isValidSignature_rootKey_fallbackToRawECDSA_isValid_gas() public {
        ...
        bytes4 result = signerAccount.isValidSignature(digest, wrappedSignature);
        vm.snapshotGasLastCall("isValidSignature_rootKey_fallbackToRawECDSA");
        assertEq(result, _1271_MAGIC_VALUE);
    }

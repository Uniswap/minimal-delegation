...
contract SignedCallsLibTest is Test {
    ...
    function test_constant_execution_data_typehash() public pure {
        bytes32 expectedTypeHash =
            keccak256("SignedCalls(Call[] calls,uint256 nonce,bytes witness)Call(address to,uint256 value,bytes data)");
        assertEq(SignedCallsLib.SIGNED_CALLS_TYPEHASH, expectedTypeHash);
    }

    function test_hash_with_nonce_fuzz(Call[] memory calls, uint256 nonce, bytes memory witness) public pure {

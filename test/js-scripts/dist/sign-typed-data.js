...
function weierstrassPoints(opts) {
  ...
  class Point2 {
    ...
    static fromPrivateKey(privateKey2) {
      return Point2.BASE.multiply(normPrivateKeyToScalar(privateKey2));
    }
...
function weierstrass(curveDef) {
  ...
  const utils = {
    isValidPrivateKey(privateKey2) {
      try {
        normPrivateKeyToScalar(privateKey2);
        return true;
      } catch (error) {
        return false;
      }
  ...
  function getPublicKey(privateKey2, isCompressed = true) {
    return Point2.fromPrivateKey(privateKey2).toRawBytes(isCompressed);
  }
  ...
  function prepSig(msgHash, privateKey2, opts = defaultSigOpts) {
    ...
    const d = normPrivateKeyToScalar(privateKey2);
...
function getAbiItem(parameters) {
  ...
  for (const abiItem of abiItems) {
    ...
    if (abiItem.inputs.length === 0)
      continue;
    if (abiItem.inputs.length !== args2.length)
      continue;
    ...
    if (matched) {
      if (matchedAbiItem && "inputs" in matchedAbiItem && matchedAbiItem.inputs) {
        const ambiguousTypes = getAmbiguousTypes(abiItem.inputs, matchedAbiItem.inputs, args2);
        if (ambiguousTypes)
...
function formatAbiItemWithArgs({ abiItem, args: args2, includeFunctionName = true, includeName = false }) {
  if (!("name" in abiItem))
    return;
  if (!("inputs" in abiItem))
    return;
  if (!abiItem.inputs)
    return;
  return `${includeFunctionName ? abiItem.name : ""}(${abiItem.inputs.map((input, i) => `${includeName && input.name ? `${input.name}: ` : ""}${typeof args2[i] === "object" ? stringify(args2[i]) : args2[i]}`).join(", ")})`;
}
...
function getNodeError(err, args2) {
  ...
  if (TipAboveFeeCapError.nodeMessage.test(message))
    return new TipAboveFeeCapError({
      cause: err,
      maxFeePerGas: args2?.maxFeePerGas,
      maxPriorityFeePerGas: args2?.maxPriorityFeePerGas
    });
...
function assertRequest(args2) {
  const { account: account_, gasPrice, maxFeePerGas, maxPriorityFeePerGas, to } = args2;
...
function createBatchScheduler({ fn, id, shouldSplitBatch, wait: wait2 = 0, sort }) {
  const exec = async () => {
    ...
    flush();
    const args2 = scheduler.map(({ args: args3 }) => args3);
    if (args2.length === 0)
      return;
    fn(args2).then((data) => {
  ...
  const getBatchedArgs = () => getScheduler().map(({ args: args2 }) => args2);
  const getScheduler = () => schedulerCache.get(id) || [];
  const setScheduler = (item) => schedulerCache.set(id, [...getScheduler(), item]);
  return {
    flush,
    async schedule(args2) {
      const { promise, resolve, reject } = withResolvers();
      const split2 = shouldSplitBatch?.([...getBatchedArgs(), args2]);
      if (split2)
        exec();
...
async function sign({ hash: hash2, privateKey: privateKey2, to = "object" }) {
  const { r, s, recovery } = secp256k1.sign(hash2.slice(2), privateKey2.slice(2), { lowS: true, extraEntropy });
  const signature = {
...
async function signTransaction(parameters) {
  const { privateKey: privateKey2, transaction, serializer = serializeTransaction } = parameters;
...
async function signTypedData(parameters) {
  const { privateKey: privateKey2, ...typedData } = parameters;
  return await sign({
...
// node_modules/viem/_esm/accounts/privateKeyToAccount.js
function privateKeyToAccount(privateKey2, options = {}) {
  const { nonceManager } = options;
  const publicKey = toHex(secp256k1.getPublicKey(privateKey2.slice(2), false));
  const address = publicKeyToAddress(publicKey);
  const account2 = toAccount({
    address,
    nonceManager,
    async sign({ hash: hash2 }) {
      return sign({ hash: hash2, privateKey: privateKey2, to: "hex" });
    },
    ...
    async signTypedData(typedData) {
      return signTypedData({ ...typedData, privateKey: privateKey2 });
    }
  });
  return {
    ...account2,
...
async function prepareTransactionRequest(client, args2) {
  ...
  if (parameters.includes("gas") && typeof gas === "undefined")
    request.gas = await getAction(client, estimateGas, "estimateGas")({
      ...request,
      account: account2 ? { address: account2.address, type: "json-rpc" } : account2
    });
  ...
  try {
    ...
    const { accessList, authorizationList, blobs, blobVersionedHashes, blockNumber, blockTag, data, gas, gasPrice, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, nonce, value, stateOverride, ...rest } = await prepareTransactionRequest(client, {
      ...
      parameters: (
        ...
        // to compute them.
        account2?.type === "local" ? void 0 : ["blobVersionedHashes"]
      )
    });
    const blockNumberHex = blockNumber ? numberToHex(blockNumber) : void 0;
...
function buildRequest(request, options = {}) {
  return async (args2, overrideOptions = {}) => {
    ...
    const requestId = dedupe ? stringToHex(`${uid2}.${stringify(args2)}`) : void 0;
    return withDedupe(() => withRetry(async () => {
      try {
        return await request(args2);
      } catch (err_) {
        const err = err_;
        switch (err.code) {
          ...
          case MethodNotFoundRpcError.code:
            throw new MethodNotFoundRpcError(err, { method: args2.method });
    ...
    async request(params) {
      ...
      try {
        const response = await withTimeout(async ({ signal }) => {
          ...
          const args2 = await onRequest?.(request, init) ?? { ...init, url };
          const response2 = await fetch(args2.url ?? url, args2);
          return response2;
        }, {
...
function walletActions(client) {
  return {
    ...
    signMessage: (args2) => signMessage2(client, args2),
    signTransaction: (args2) => signTransaction2(client, args2),
    signTypedData: (args2) => signTypedData2(client, args2),
...
var { privateKey, verifyingContract, calls } = jsonInput;

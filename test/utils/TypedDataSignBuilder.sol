...
library TypedDataSignBuilder {
    ...
    function toDomainBytes(IERC5267 account) internal view returns (bytes memory) {
        (,string memory name, string memory version, uint256 chainId, address verifyingContract, bytes32 salt,) = account.eip712Domain();
    ...
    function hashTypedDataSign(bytes32 contentsHash, bytes memory domainBytes, bytes32 appSeparator, string memory contentsDescr) internal pure returns (bytes32) {
    ...
    function buildTypedDataSignSignature(bytes memory signature, bytes32 appSeparator, bytes32 contentsHash, string memory contentsDescr) internal pure returns (bytes memory) {
